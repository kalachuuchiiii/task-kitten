import { comparisonOperatorsMap } from "@/constants";
...
export class TaskServices {
  ...
  updateTask = async ({
    ...
    return await runWithSession(async (session) => {
      const newRecord = await new TaskRecord({
  ...
  getTaskHistory = async ({
    ...
    const {
      ...
      nextPage,
    } = await taskRecordHelper.getListOfResource(
      { taskId: taskId },
      page,
      limit,
      sort
  ...
  createTask = async (taskForm: TaskFields) => {
    return await runWithSession(async (session) => {
      const newValues = Object.entries(taskForm)
        .filter(([key, val]) =>
          taskRecordAllowedFields.includes(
            key as (typeof taskRecordAllowedFields)[number]
          )
        )
        .map(([key, value]) => ({
          field: key,
        ...
        const due = new Date(taskForm.due);
        due.setHours(0,0,0,0);
        const startedAt = new Date(taskForm.startedAt);
  ...
  getTaskList = async ({
    userId,
    filters,
  }: Omit<IDS<{ filters: FilterTaskType }>, "taskId">) => {
    const {
      page,
      limit,
      sort,
    ...
    if (due.specific.date && due.specific.operator) {
      const op = comparisonOperatorsMap[due.specific.operator];
      filterQuery.due = { [op]: due.specific.date };
    } else if (due.range.to && due.range.from) {
      filterQuery.due = { $lte: due.range.to, $gte: due.range.from };
    }

    if (startedAt.specific.date && startedAt.specific.operator) {
      const op = comparisonOperatorsMap[startedAt.specific.operator];
      filterQuery.startedAt = { [op]: startedAt.specific.date };
    } else if (startedAt.range.from && startedAt.range.to) {
      filterQuery.due = {
        $lte: startedAt.range.to,
        $gte: startedAt.range.from,
      };
    }

    if (startedAt.specific.date) {
      const op = comparisonOperatorsMap[startedAt.specific.operator];
      filterQuery.startedAt = { [op]: startedAt.specific.date };
    }
